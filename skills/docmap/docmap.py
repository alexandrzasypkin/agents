...
def keep(p):
    ...
    if ".agents" in parts:                       # library infra copy, not project docs — except plans/
...
def md_files(root):
    ...
    if root != ".":                              # scope to the given path prefix
        files = [f for f in files if os.path.normpath(f) == root
                 or os.path.normpath(f).startswith(root + os.sep)]
...
def parse(path):
    ...
    m = FM.match(text)
    if m:
        d = parse_fm(m.group(1)); d["has_fm"] = True; body = text[m.end():]
    else:
        d = {"type": "", "status": "", "tags": [], "has_fm": False}; body = text
    h1 = ""
...
def dangling(path, body):
    ...
    for m in LINK.finditer(body):
        tgt = m.group(1).strip().split()[0] if m.group(1).strip() else ""   # drop a ` "title"` suffix
        if not tgt or re.match(r'^(https?:|mailto:|tel:|#|/|data:)', tgt):
            continue
...
def main():
    ...
    while i < len(args):
        a = args[i]
        if a == "--type" and i + 1 < len(args):     ftype = args[i + 1]; i += 2
        elif a == "--tag" and i + 1 < len(args):    ftag = args[i + 1]; i += 2
        elif a == "--status" and i + 1 < len(args): fstatus = args[i + 1]; i += 2
        else:                                        root = a; i += 1
    ...
    for f in md_files(root):
        ...
        if d:
            broken.append((f, d))
        if ftype and info["type"] != ftype:       continue
        if fstatus and info["status"] != fstatus: continue
        if ftag and ftag not in info["tags"]:     continue
    ...
    if missing:
        print(f"\n[docmap] {len(missing)} doc(s) with NO frontmatter — invisible to --type/--tag/--status "
              f"(the filter lies without a signal):", file=sys.stderr)
        for f in missing:
            print(f"  {f}", file=sys.stderr)
    if broken:
        n = sum(len(d) for _, d in broken)
        print(f"\n[docmap] {n} dangling link(s) in {len(broken)} doc(s) — target file missing:", file=sys.stderr)
        for f, d in broken:
            for t in d:
